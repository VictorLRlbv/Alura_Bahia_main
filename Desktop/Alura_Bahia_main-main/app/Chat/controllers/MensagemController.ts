...
export class MensagemController {
    ...
    private _resposta: MensagemRespostaView
    ...
    verificaResposta(mensagem: string, data: string) {
        let messageComplete = ''
        if(
            //#region 
        ...
        if(
            mensagem.toUpperCase().split(' ').some(item => item == 'GAMES') ||
        ...
        if(
            mensagem.toUpperCase().split(' ').some(item => item == 'BELEZA') ||
    ...
    responde(mensagensResposta: string, data: string) {
        this._resposta.update(mensagensResposta, data)
    }
    criaResposta(msgtxt: string, data: string) {
